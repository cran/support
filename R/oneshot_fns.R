sp <- function(n, p, ini=NA,
  ...
  if (std.flg){
    ...
    if (any(is.na(ini))){
      ini.flg <- FALSE
      if (p==1){
        ini <- matrix(randtoolbox::sobol(n,p,scrambling=TRUE,seed=sample(1e6,1)),ncol=1)
      }else{
        ini <- randtoolbox::sobol(n,p,scrambling=TRUE,seed=sample(1e6,1))
      }
    }
    ...
    dist.param <- list(NA)
    if (any(is.na(ini))){
      ini <- matrix(jitter(dist.samp[sample(1:nrow(dist.samp),n,FALSE),]),ncol=p)
      # print(ini)
      for (i in 1:p){ #enforce bounds
...
psp <- function(n, p,
  ...
  if (std.flg){
    ...
    dist.param <- list(NA)
    if (any(is.na(ini))){
      ini <- jitter(dist.samp[sample(1:nrow(dist.samp),n,FALSE),])
    }else{
      ini <- sweep(sweep(ini,2,mmpts,"-"),2,sdpts,"/")
    }
    if (p==1){
