sp_seq <- function(D, nseq, ini=NA, num.rep=1,
  ...
  for (jj in 1:num.rep){
    ...
    if (std.flg){
      ...
      if (any(is.na(ini))){
        if (p==1){
          ini <- matrix(randtoolbox::sobol(nseq,p,scrambling=TRUE,seed=sample(1e6,1)),ncol=1)
        }else{
          # print(nseq)
          ini <- randtoolbox::sobol(nseq,p,scrambling=TRUE,seed=sample(1e6,1))
        }
      ...
      if (any(is.na(bd))){
        bd <- matrix(NA,nrow=p,ncol=2,byrow=TRUE)
        bd.flg <- FALSE
      }
      for (i in 1:p){
      ...
      #Set initial points
      ini <- jitter(dist.samp[sample(1:nrow(dist.samp),nseq,FALSE),])
      if (p==1){
...
herd <- function(nseq, p, 
  ...
  # for (jj in 1:num.rep){
    ...
    if (std.flg){
          ...
          if (warm.ini){
            ...
            for (jj in 1:p){
              ...
              res <- stats::optim( par=iniptd, method="L-BFGS-B",
                                     fn=function(xx){-herdobj_seq(xx,matrix(des[,jj],ncol=1),matrix(dist.subsamp[,jj],ncol=1),0.1)}, 
                                     gr=function(xx){-herdgrad_seq(xx,matrix(des[,jj],ncol=1),matrix(dist.subsamp[,jj],ncol=1),0.1)},
      ...
      dist.param <- list(NA)
      if (any(is.na(ini))){
        ini <- dist.samp[sample(1:nrow(dist.samp),nseq,FALSE),]
